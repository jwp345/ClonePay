...
public class IncreaseMoneyRequestService implements IncreaseMoneyRequestUseCase {
    ...
    public MoneyChangingRequest increaseMoneyRequest(IncreaseMoneyRequestCommand command) {
        ...
        // 2. 고객의 연동된 계좌가 있는지, 고객의 연동된 계좌의 잔액이 충분한지, 그리고 정상적인지 확인(뱅킹)

        // 3. 법인 계좌 상태도 정상인지도 확인(뱅킹)

        // 4. 증액을 위한 "기록", 요청 상태로 MoneyChangingRequest 를 생성한다. (MoneyChangingRequest)

        // 5. 펌뱅킹을 수행하고 (고객의 연동된 계좌 -> 페캠페이 법인 계좌) (뱅킹)
    ...
    public MoneyChangingRequest increaseMoneyRequestAsync(IncreaseMoneyRequestCommand command) {
        ...
        SubTask validBankingAccountTask = SubTask.builder()
                ...
                .status("ready")
                .build();
        // Amount Money Firmbanking --> 무조건 ok 받았다고 가정. 추후에 subtask로?
        ...
        // 받은 응답을 다시, CountDownLatchManger를 통해서 결과 데이터를 받음
